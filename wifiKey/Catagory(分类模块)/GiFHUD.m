...
//singleton_implementation(GiFHUD);
...
- (instancetype)initWithView:(UIView *)view  {
    if ((self = [super initWithFrame:view.bounds])) {
        ...
        [self.layer setBackgroundColor:[[UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:0.3] CGColor]];
        
        
        self.imageView = [[UIImageView alloc] initWithFrame:CGRectMake((view.frame.size.width-Size)/2, (view.frame.size.height-Size)/2-Size, Size, Size)];
        [self addSubview:self.imageView];
        [self setGifWithImageName:@"loading.gif"];
...
- (void)dismiss {
//    if (![[self sharedGiFHUD] shown])
//        return;
//    [[[self sharedGiFHUD] overlay] removeFromSuperview];
    [self fadeOut];
}
...
- (UIView *)overlay {
    ...
    if (!self.overlayView) {
        self.overlayView = [[UIView alloc] initWithFrame:self.superview.frame];
...
- (void)setGifWithImages:(NSArray *)images {
    ...
    [[self imageView] setAnimationDuration:GifSpeed];
}

- (void)setGifWithImageName:(NSString *)imageName {
    [[self imageView] stopAnimating];
    [[self imageView] setImage:[UIImage animatedImageWithAnimatedGIFURL:[[NSBundle mainBundle] URLForResource:imageName withExtension:nil]]];
}

- (void)setGifWithURL:(NSURL *)gifUrl {
    [[self imageView] stopAnimating];
    [[self imageView] setImage:[UIImage animatedImageWithAnimatedGIFURL:gifUrl]];
}
